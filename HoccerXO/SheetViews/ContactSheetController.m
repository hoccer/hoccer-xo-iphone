...
- (id) valueForItem: (DatasheetItem*) item {
    if ([item isEqual: self.chatItem]) {
        ...
        NSFetchRequest *fetchRequest = [MediaBrowserDataSource fetchRequestForContact:self.contact withMediaTypes:[Attachment allMediaTypes]  managedObjectModel:AppDelegate.instance.managedObjectModel];
...
- (void) configureCell: (DatasheetCell*) aCell withItem: (DatasheetItem*) item atIndexPath: (NSIndexPath*) indexPath {
    if ([aCell.reuseIdentifier isEqualToString: @"SmallContactCell"]) {
        ...
        cell.subtitleLabel.text   = isInvited ? NSLocalizedString(@"group_membership_state_invited", nil) : isAdmin ? NSLocalizedString(@"group_membership_role_admin", nil) : nil;
